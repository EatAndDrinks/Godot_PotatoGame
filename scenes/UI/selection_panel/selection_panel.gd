...
func _on_player_selected(player : UnitStats) -> void:
	...
	player_description.text = "[code]生命值:[color=green]%s[/color]\n
	伤害值: [color=green]%s[/color]\n
	速度:[color=green]%s[/color]\n
	幸运：[color=green]%s[/color]\n
	防御概率:[color=green]%s%%[/color][/code]" % [player.health,player.damage , player.speed , player.luck , player.block_chance]

func _on_weapon_selected(weapon : ItemWeapon) -> void:
	Global.main_weapo_selected = weapon
...
func _on_continue_button_pressed() -> void:
	...
	if not Global.main_player_selected and not Global.main_weapo_selected:
		return
