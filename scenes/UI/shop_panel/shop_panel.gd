...
func _on_combine_button_pressed() -> void:
	...
	var weapons_to_remove : Array[Weapon] = Global.player.current_weapon.filter(func(w: Weapon):
	...
	for weapon : Weapon in weapons_to_remove:
		Global.player.current_weapon.erase(weapon)
	...
	#创建升级后的武器
	var upgrade_weapon : ItemWeapon = load(clicked_weapon.upgrade_to.resource_path)
	Global.player.add_weapon(upgrade_weapon)
	Global.equipped_weapons.append(upgrade_weapon)
	#创建升级后武器卡片
...
func _on_sell_button_3_pressed() -> void:
	...
	var weapon_to_remove : Weapon = Global.player.current_weapon.filter(func(w : Weapon):
		return w.data.item_name == clicked_weapon.item_name).front()
	#删除武器
	if weapon_to_remove:
		Global.player.current_weapon.erase(weapon_to_remove)
		Global.equipped_weapons.erase(weapons_container)
